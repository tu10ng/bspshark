...
async fn crud_pitfall() {
    ...
    let id = pitfall["id"].as_str().unwrap().to_string();
    assert_eq!(pitfall["status"], "active");
    assert_eq!(pitfall["severity"], "high");

    // Get with references
    let req = test::TestRequest::get()
        .uri(&format!("/api/v1/pitfalls/{}", id))
        .to_request();
    let resp = test::call_service(&app, req).await;
    ...
    let req = test::TestRequest::get()
        .uri("/api/v1/pitfalls?status=resolved")
        .to_request();
    let resp = test::call_service(&app, req).await;
    ...
    let req = test::TestRequest::get()
        .uri("/api/v1/pitfalls?status=active")
        .to_request();
    let resp = test::call_service(&app, req).await;
    let pitfalls: Vec<serde_json::Value> = test::read_body_json(resp).await;
    assert_eq!(pitfalls.len(), 0);
...
async fn pitfall_fts_search() {
    ...
    for (title, desc) in [
        ...
        let req = test::TestRequest::post()
            ...
            .to_request();
        let resp = test::call_service(&app, req).await;
        assert_eq!(resp.status(), 201, "Failed to create pitfall: {}", title);
    }

    // Verify all 3 were created
    let req = test::TestRequest::get().uri("/api/v1/pitfalls").to_request();
    let resp = test::call_service(&app, req).await;
    let all: Vec<serde_json::Value> = test::read_body_json(resp).await;
    assert_eq!(all.len(), 3, "Expected 3 pitfalls, got {}", all.len());

    // FTS search for "数据库"
    let req = test::TestRequest::get()
        .uri("/api/v1/pitfalls?q=%E6%95%B0%E6%8D%AE%E5%BA%93")
        .to_request();
    let resp = test::call_service(&app, req).await;
    ...
    let req = test::TestRequest::get()
        .uri("/api/v1/pitfalls?q=%E7%BC%93%E5%AD%98")
        .to_request();
    let resp = test::call_service(&app, req).await;
    let results: Vec<serde_json::Value> = test::read_body_json(resp).await;
    assert_eq!(results.len(), 1);
